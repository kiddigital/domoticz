...
Volvo implementation of VehicleApi baseclass
Author: KidDigital (github.com/kiddigital)

11/03/2021 1.0 Creation
...
#define VOLVO_REFRESHTOKEN_USERVAR "volvooncall_refreshtoken"

CVolvoApi::CVolvoApi(const std::string &username, const std::string &password, const std::string &vinnr)
{
	m_username = username;
...
bool CVolvoApi::Login()
{
	bool bSuccess = false;
	std::string szLastUpdate = TimeToString(nullptr, TF_DateTime);

	if (m_refreshtoken.empty() || m_refreshtoken == VOLVO_REFRESHTOKEN_CLEARED)
	{
		_log.Log(LOG_NORM, "VolvoApi: Attempting login (using provided Authorization code).");

		m_authenticating = true;

		if (GetAuthToken(m_username, m_authtoken, false))
		{
			_log.Log(LOG_NORM, "VolvoApi: Login successful.");
		...
		{
			_log.Log(LOG_ERROR, "VolvoApi: Login unsuccessful!");
		}
	...
	{
		_log.Log(LOG_NORM, "VolvoApi: Attempting (re)login (using stored refresh token!).");
...
bool CVolvoApi::RefreshLogin()
{
	bool bSuccess = false;
	std::string szLastUpdate = TimeToString(nullptr, TF_DateTime);

	_log.Debug(DEBUG_NORM, "VolvoApi: Refreshing login credentials.");
	m_authenticating = true;

	if (GetAuthToken("", "", true))
	{	
		_log.Log(LOG_NORM, "VolvoApi: Login credentials Refresh successful.");
		bSuccess = true;
	}
	else
	{
		_log.Log(LOG_ERROR, "VolvoApi: Failed to refresh login credentials.");
		m_accesstoken = "";
		if (!m_refreshtoken.empty())
		{
			m_refreshtoken = VOLVO_REFRESHTOKEN_CLEARED;
		}
	}
...
bool CVolvoApi::GetAllData(tAllCarData& data)
{
	bool bSucces = false;
...
bool CVolvoApi::GetVehicleData(tVehicleData& data)
{
	Json::Value reply;
	bool bData = false;
...
bool CVolvoApi::GetCustomData(tCustomData& data)
{
	Json::Value reply;
	...
	if (m_capabilities.has_custom_data)
	{
		...
		if(m_fieldcnt < 0)
		{
			...
			_log.Debug(DEBUG_NORM, "VolvoApi: Reset Customfield count to %d", m_fieldcnt);
		}
		else
		{
			if (GetResourceData(strarray[m_fieldcnt], reply))
			{
				if (reply.empty())
				{
					_log.Debug(DEBUG_NORM, "VolvoApi: Got empty data for resource %s", strarray[m_fieldcnt].c_str());
				}
				else
				{
					_log.Debug(DEBUG_RECEIVED, "VolvoApi: Got data for resource %s :\n%s", strarray[m_fieldcnt].c_str(),reply.toStyledString().c_str());
					...
					if (!reply[strarray[m_fieldcnt]].empty())
					{
						if (reply[strarray[m_fieldcnt]].isMember("value"))
						{
							...
							_log.Debug(DEBUG_NORM, "VolvoApi: Got data for resource (%d) %s : %s", m_fieldcnt, strarray[m_fieldcnt].c_str(), resourceValue.c_str());
						}
					}
				}
			}
			...
			{
				_log.Debug(DEBUG_NORM,"VolvoApi: Failed to retrieve data for resource %s!", strarray[m_fieldcnt].c_str());
			}
		}
	}
...
bool CVolvoApi::GetResourceData(const std::string &datatype, Json::Value &reply)
{
	...
	if (!SendToApi(Get, _sUrl, "", _sResponse, *(new std::vector<std::string>()), reply, true, (VOLVO_APITIMEOUT / 2)))
	{
		_log.Log(LOG_ERROR, "VolvoApi: Failed to get resource data %s.", datatype.c_str());
		return false;
	}

	_log.Debug(DEBUG_NORM, "VolvoApi: Get resource data %s received reply: %s", datatype.c_str(), _sResponse.c_str());

	return true;
}

bool CVolvoApi::IsAwake()
{
	// Current Volvo on Call (API) does not have an 'Awake' state
	// So we fake one, we just request the list of all available resources that are available for the current (BYO)CAR

	std::stringstream ss;
	ss << VOLVO_URL << VOLVO_API << "/" << m_VIN << "/resources";
	std::string _sUrl = ss.str();
	std::string _sResponse;
	Json::Value _jsRoot;
	bool is_awake = false;
	...
	{
		is_awake = true;
		_log.Debug(DEBUG_NORM, "VolvoApi: Awake state checked. We are awake.");
	}

	return(is_awake);
}

bool CVolvoApi::ProcessAvailableResources(Json::Value& jsondata)
{
	...
	catch(const std::exception& e)
	{
		...
		_log.Log(LOG_ERROR, "VolvoApi: Crashed during processing of resources: %s", what.c_str());
	}

	return bProcessed;
}

bool CVolvoApi::SendCommand(eCommandType command, std::string parameter)
{
...
bool CVolvoApi::SendCommand(const std::string &command, Json::Value &reply, const std::string &parameters)
{
	/*
	std::stringstream ss;
	if (command == "wake_up")
	...
	{
		_log.Log(LOG_ERROR, "VolvoApi: Failed to send command %s.", command.c_str());
		return false;
	}

	//_log.Log(LOG_NORM, "VolvoApi: Command %s received reply: %s", command.c_str(), _sResponse.c_str());
	_log.Debug(DEBUG_NORM, "VolvoApi: Command %s received reply: %s", command.c_str(), _sResponse.c_str());
	*/
	return true;
}
...
bool CVolvoApi::GetAuthToken(const std::string &username, const std::string &password, const bool refreshUsingToken)
{
	...
	{
		_log.Log(LOG_ERROR, "VolvoApi: No username specified.");
		return false;
	}
	if (!refreshUsingToken && username.empty())
	{
		_log.Log(LOG_ERROR, "VolvoApi: No password specified.");
		return false;
	}

	if (refreshUsingToken && (m_refreshtoken.empty() || m_refreshtoken == VOLVO_REFRESHTOKEN_CLEARED))
	{
		_log.Log(LOG_ERROR, "VolvoApi: No refresh token to perform refresh!");
		return false;
	}

	std::stringstream ss;
	ss << VOLVO_URL_AUTH << VOLVO_API_TOKEN;
	std::string _sUrl = ss.str();
	std::ostringstream s;
	...
	{
		_log.Log(LOG_ERROR, "VolvoApi: Received error response (%s).", _jsRoot["error"].asString().c_str());
		return false;
	}
	...
	{
		_log.Log(LOG_ERROR, "VolvoApi: Received access token is zero length.");
		return false;
	}
	...
	if (m_refreshtoken.empty())
	{
		_log.Log(LOG_ERROR, "VolvoApi: Received refresh token is zero length.");
		return false;
	}
	...
	std::string& sResponse, const std::vector<std::string>& vExtraHeaders, Json::Value& jsDecodedResponse, const bool bSendAuthHeaders, const int timeout)
{
	...
	{
		_log.Log(LOG_ERROR, "VolvoApi: No access token available.");
		return false;
	}
	...
	try
	{
		...
		if(timeout == 0)
		{
			HTTPClient::SetConnectionTimeout(VOLVO_APITIMEOUT);
			HTTPClient::SetTimeout(VOLVO_APITIMEOUT);
		}
		else
		{
		...
		_log.Debug(DEBUG_RECEIVED, "VolvoApi: Performing request to Api: %s", sUrl.c_str());
		...
		default:
			{
				_log.Log(LOG_ERROR, "VolvoApi: Unknown method specified.");
				return false;
			}
		}
		...
		if (sResponse.empty())
		{
			_log.Log(LOG_ERROR, "VolvoApi: Received an empty response from Api (HTTP %d).", m_httpresultcode);
			return false;
		}

		if (!ParseJSon(sResponse, jsDecodedResponse))
		{
			_log.Log(LOG_ERROR, "VolvoApi: Failed to decode Json response from Api (HTTP %d).", m_httpresultcode);
			return false;
		}
	}
	...
	{
		std::string what = e.what();
		_log.Log(LOG_ERROR, "VolvoApi: Error sending information to Api: %s", what.c_str());
		return false;
	}
	return true;
}
...
uint16_t CVolvoApi::ExtractHTTPResultCode(const std::string& sResponseHeaderLine0)
{
